...
function getParticipantsList(meeting) {
  return Array.from(meeting.participants.values()).map(p => ({
    ...
    online: clients.has(p.id) && clients.get(p.id)?.readyState === 1,
  }));
}

function getContentState(meeting) {
  if (!meeting.content) return null;
  return {
    type: meeting.content.type,
    title: meeting.content.title,
    url: meeting.content.url || null,
    ownerId: meeting.content.ownerId,
    remoteHolderId: meeting.content.remoteHolderId,
    scrollMode: meeting.content.scrollMode || 'uniform',
    scroll: meeting.content.scroll || null,
    page: meeting.content.page || null,
    media: meeting.content.media || null,
    fileMeta: meeting.content.fileMeta || null,
  };
}
